...
class SOMVAE:
    ...
    def __init__(self, inputs, latent_dim=72, som_dim=[16,16], learning_rate=1e-4, decay_factor=0.95, decay_steps=1000,
            input_length=12, input_channels=27, alpha=1., beta=1., gamma=1., tau=1.):
    ...
    def z_dist_flat(self):
        ...
        z_dist = tf.squared_difference(tf.expand_dims(tf.expand_dims(self.z_e, axis = 1), axis = 1), 
    ...
    def reconstruction_q(self):
        ...
        with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE):
            ...
            h_reshaped = tf.reshape(h_flat_dec, [-1, 3, 3, 1])
            h_unpool1 = tf.keras.layers.UpSampling2D((2,3))(h_reshaped)
            h_deconv1 = tf.nn.relu(conv2d(h_unpool1, [4,4,1,1], "deconv1"))
            
            h_unpool2 = tf.keras.layers.UpSampling2D((2,3))(h_deconv1)
            h_deconv2 = tf.nn.sigmoid(conv2d_transposed(h_unpool2, [4,4,1,1], outshape=[self.batch_size, 12, 27, 1], name = "deconv2"))
