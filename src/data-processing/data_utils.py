...
def conversion_to_block(df: pd.DataFrame, id_key: str, rule: str,
    ...
    assert df[id_key].is_monotonic and df.groupby(id_key).apply(lambda x:x["time_to_end"].is_monotonic_decreasing).all()
...
class data_processor():
    def __init__(self, id_column='subject_id', time_column='charttime', feature_set_names='vitals', fill_limit=None,
                 mask_value=0.0,
